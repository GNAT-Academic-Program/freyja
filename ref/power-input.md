...
`Q2`, the 5 V pass switch, has a 100nF gate-source capacitor because closing
it connects VBUS to 66µF of +5V bulk. R49 is 47k against a 100k gate-source
pull-up: its gate RC time constant is approximately 3.20ms. The divider
limits VGS to -6.12V at a 9V firmware fault, within the ±8V gate rating.
Firmware must still keep Q2 off at 9V to protect the +5V loads.
...
| `D8` | SMBJ15A-13-F | transient clamp on VIN_EXT at J50, ahead of D4 |
...
## Screw-terminal transient clamp

D8 (SMBJ15A-13-F) connects VIN_EXT to GND, cathode toward J50 positive,
before the OR-ing diode D4. Place it next to J50 with a short, wide return
to terminal ground. Its 15V standoff permits the specified 9–14V input;
its rated pulse clamp is 24.4V, not 15V. This is transient suppression,
not protection against an indefinitely applied 24V supply. Source fault
current and surge energy must remain within the protection path's ratings.
